...
void print_usage() {
    println("--help");
    println(R"s(
        ./k_means_cpp_cpu_parallel --input_points_path path/to/input_points.csv
                                [--num_of_threads 15]
...
int main(int argc, char **argv) {
    ...
    auto kmeans_default_params = ni::cpu_parallel::KMeansParams::get_default();
    ...
    timer.stop_and_log();
    auto kmeans = ni::cpu_parallel::KMeans<float>(kmeans_default_params);
    ...
    if (benchmark_dir.has_value) {
        ...
        println(fast_io::dbg(), timer.get_log());
