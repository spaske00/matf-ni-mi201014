...
template<std::integral T>
inline void write(basic_atl_io_observer<T> hd,io_scatters_t scatters)
{
...
public:
	...
	basic_atl_file(basic_atl_file const& mcf):basic_atl_io_observer<T>{mcf.phandle->Duplicate()}{}
	...
	basic_atl_file(basic_atl_file&& mcf) noexcept:basic_atl_io_observer<T>{mcf.phandle}
	{
		mcf.phandle=nullptr;
	}
