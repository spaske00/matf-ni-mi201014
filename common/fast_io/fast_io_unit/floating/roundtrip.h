...
inline constexpr m10_result<typename iec559_traits<flt>::mantissa_type> dragonbox_main(typename iec559_traits<flt>::mantissa_type m2,std::int_least32_t e2) noexcept
{
	...
	if constexpr(std::same_as<flt,double>)
	{
		...
		std::uint_least64_t q{zi/big_divisor};
		std::uint_least64_t r{zi%big_divisor};
		if(r<delta)
		{
		...
		std::uint_least32_t const dist_q{dist / 100};
		std::uint_least32_t const dist_q_mul100{dist_q * 100};
		q+=dist_q;
...
inline constexpr Iter fixed_case1_integer_and_point(Iter iter,typename iec559_traits<flt>::mantissa_type m10,std::int_least32_t olength,std::int_least32_t real_exp) noexcept
{
	...
	{
		auto tmp{iter};
		print_reserve_integral_main_impl<10,false>(iter+=olength+1,m10,static_cast<std::uint_least32_t>(olength));
...
inline constexpr Iter print_rsv_fp_decision_impl(Iter iter,typename iec559_traits<flt>::mantissa_type m10,std::int_least32_t e10) noexcept
{
	...
	if constexpr(mt==::fast_io::manipulators::floating_format::scientific)
	{
		...
		else
		{
			...
			iter=print_rsv_fp_general_scientific_common_impl<comma>(iter,m10,olength);
		}
